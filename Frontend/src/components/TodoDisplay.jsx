...
function TodoDisplay({ todos, onToggle, onEdit, onDelete }) {
	...
	async function handleTodoCardClick(e) {
		const todoId =
			e.target.parentElement.getAttribute('data_id') ||
		...
		return (
			<div
				...
				<div className='flex gap-5'>
					<button
						className='rounded-2xl bg-blue-400 text-white py-1.5 px-5 hover:cursor-pointer hover:bg-blue-500'
						onClick={handleEdit}>
						Edit
					</button>
					<button
						className='rounded-2xl bg-red-400 text-white py-1.5 px-3 hover:cursor-pointer hover:bg-red-500'
	...
	return (
		<>
			<div className='flex my-10 gap-10 w-full flex-col'>
